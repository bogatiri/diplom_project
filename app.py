    ...
    if request.method == "POST":
        ...
        try:
            ...
            existing_section = (
                ...
                .filter_by(name_of_section="Default Section", user=new_user)
                .first()
            )
        ...
        if user and bcrypt.check_password_hash(user.password, password_old):
            if password_new == password_new_repeated:
                ...
                response = make_response(
                    render_template("main.html", user_data=user_data)
                )
                response.set_cookie("user", user.email, max_age=3600 * 24, path="/")
                return response
            else:
...
def save_name_of_section():
    if request.method == "POST":
        ...
        if user:
            section = (
                db_session.query(Section).filter_by(user=user, id=section_id).first()
            )
            if section:
...
def add_section():
    try:
        ...
        if user:
            section = (
                db_session.query(Section)
                .filter_by(user=user, name_of_section=section_name)
                .first()
            )
            if not section:
...
def get_sections():
    try:
        ...
        if user:
            ...
            sections_json = [
                {"id": section.id, "name_of_section": section.name_of_section}
